...
SELECT
    ...
    CASE
        WHEN enrichment->'dshield'->'ip'->>'attacks' IS NULL THEN 'no_data'
        WHEN CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER) = 0 THEN '0_first_time'
        WHEN CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER) BETWEEN 1 AND 10 THEN '1-10_low'
        WHEN CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER) BETWEEN 11 AND 100 THEN '11-100_medium'
        WHEN CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER) BETWEEN 101 AND 1000 THEN '101-1000_high'
        WHEN CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER) > 1000 THEN '1000+_very_high'
        ELSE 'unknown'
    ...
    CASE
        WHEN enrichment->'dshield'->'ip'->>'attacks' IS NULL THEN NULL
        ELSE ROUND(AVG(CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER)), 2)
    END as avg_attack_count,

    -- Max attack count within bucket
    CASE
        WHEN enrichment->'dshield'->'ip'->>'attacks' IS NULL THEN NULL
        ELSE MAX(CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER))
...
ORDER BY
    CASE
        WHEN enrichment->'dshield'->'ip'->>'attacks' IS NULL THEN 0
        WHEN CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER) = 0 THEN 1
        WHEN CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER) BETWEEN 1 AND 10 THEN 2
        WHEN CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER) BETWEEN 11 AND 100 THEN 3
        WHEN CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER) BETWEEN 101 AND 1000 THEN 4
        WHEN CAST(enrichment->'dshield'->'ip'->>'attacks' AS INTEGER) > 1000 THEN 5
        ELSE 6
    END;
