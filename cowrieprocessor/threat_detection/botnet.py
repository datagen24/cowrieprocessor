...
class BotnetCoordinatorDetector:
    ...
    def _extract_credentials(
        ...
        if raw_events:
            for event in raw_events:
                if (event.session_id == session.session_id and 
                    event.event_type == "cowrie.login.success" and
                    event.payload):
    ...
    def _extract_commands(
        ...
        if raw_events and not commands:
            for event in raw_events:
                if (event.session_id == session.session_id and 
                    event.event_type == "cowrie.command.input" and
                    event.payload):
    ...
    def _analyze_geographic_clustering(self, coordination_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        is_clustered = (country_diversity <= self.geographic_cluster_threshold or 
                       asn_diversity <= self.geographic_cluster_threshold)
