...
class UnicodeSanitizer:
    ...
    def sanitize_unicode_string(
        cls, text: str, strict: bool = False, replacement: str = '', preserve_whitespace: bool = True
    ) -> str:
        ...
        if sanitized != text:
            ...
            logger.debug(f"Sanitized Unicode control characters: {', '.join(set(control_chars_found))}")
    ...
    def _sanitize_json_object(cls, obj: Any) -> Any:
        ...
        if isinstance(obj, dict):
            return {cls._sanitize_json_object(key): cls._sanitize_json_object(value) for key, value in obj.items()}
