...
def snowshoe_analyze(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Query sessions
        sessions = _query_sessions_for_analysis(session_factory, window_start, window_end, args.sensor)
    ...
    try:
        ...
        report = {
            ...
            "summary": {
                ...
                "avg_confidence": (
                    sum(float(d.confidence_score) for d in detections) / len(detections) if detections else 0
                ),
            },
            "detections": [
                {
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    botnet_parser = subparsers.add_parser("botnet", help="Detect coordinated botnet attacks")
    ...
    botnet_parser.add_argument("--output", help="Write JSON report to file instead of stdout")
    botnet_parser.add_argument("--store-results", action="store_true", help="Store results in database")
    botnet_parser.add_argument("--db", help="Database URL or SQLite path. If omitted, reads from settings.")
    botnet_parser.add_argument("--status-dir", help="Directory for status JSON", default=None)
    botnet_parser.add_argument("--ingest-id", help="Explicit ingest identifier", default=None)
    botnet_parser.add_argument("--sensor", help="Filter analysis by specific sensor", default=None)

    # Snowshoe analyze command
    snowshoe_parser = subparsers.add_parser("snowshoe", help="Analyze sessions for snowshoe attacks")
    snowshoe_parser.add_argument("--window", default="24h", help="Analysis window (e.g., 24h, 7d) (default: 24h)")
    snowshoe_parser.add_argument(
        "--sensitivity", type=float, default=0.7, help="Detection sensitivity threshold (0.0-1.0) (default: 0.7)"
    )
    snowshoe_parser.add_argument("--sensor", help="Filter by specific sensor name")
    snowshoe_parser.add_argument("--output", help="Output file path (default: stdout)")
    snowshoe_parser.add_argument("--store-results", action="store_true", help="Store results in database")
    snowshoe_parser.add_argument("--db", help="Database URL or SQLite path")
    snowshoe_parser.add_argument("--status-dir", help="Directory for status JSON files")
    snowshoe_parser.add_argument("--ingest-id", help="Ingest identifier for status tracking")

    # Longtail analyze command
    longtail_parser = subparsers.add_parser(
        "longtail", help="Analyze sessions for rare, unusual, and emerging attack patterns"
    )
    longtail_parser.add_argument(
        "--lookback-days", type=int, default=7, help="Number of days to look back for analysis (default: 7)"
    )
    longtail_parser.add_argument(
        "--rarity-threshold",
        type=float,
    ...
    longtail_parser.add_argument(
        "--sequence-window", type=int, default=5, help="Number of commands in sequence analysis (default: 5)"
    )
    longtail_parser.add_argument(
        "--cluster-eps", type=float, default=0.3, help="DBSCAN epsilon parameter for clustering (default: 0.3)"
    )
    longtail_parser.add_argument(
        "--min-cluster-size", type=int, default=5, help="Minimum cluster size for DBSCAN (default: 5)"
    )
    longtail_parser.add_argument(
        "--entropy-threshold",
        type=float,
    ...
    longtail_parser.add_argument(
        "--batch-size", type=int, default=100, help="Number of sessions to process in each batch (default: 100)"
    )
    longtail_parser.add_argument(
        "--memory-limit-gb",
        type=float,
    ...
    longtail_parser.add_argument(
        "--detailed", action="store_true", help="Show detailed JSON output instead of summary report"
    )
    longtail_parser.add_argument("--sensor", help="Filter by specific sensor name")
    longtail_parser.add_argument("--output", help="Output file path (default: stdout)")
    longtail_parser.add_argument("--store-results", action="store_true", help="Store results in database")
    longtail_parser.add_argument("--db", help="Database URL or SQLite path")
    longtail_parser.add_argument("--status-dir", help="Directory for status JSON files")
    longtail_parser.add_argument("--ingest-id", help="Ingest identifier for status tracking")
    ...
    report_parser.add_argument("--date", help="Report date (YYYY-MM-DD) (default: last 7 days)")
    report_parser.add_argument("--min-confidence", type=float, help="Minimum confidence score for detections")
    report_parser.add_argument("--output", help="Output file path (default: stdout)")
    report_parser.add_argument("--db", help="Database URL or SQLite path")
