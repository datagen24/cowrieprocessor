...
def test_enrich_passwords_handles_missing_date_range_gracefully(temp_dir: Path, test_db: sessionmaker[Session]) -> None:
...
def test_enrich_passwords_handles_last_days_parameter_correctly(temp_dir: Path, test_db: sessionmaker[Session]) -> None:
...
def test_refresh_enrichment_handles_database_connection_gracefully(temp_dir: Path) -> None:
...
def test_track_password_creates_new_password_record(test_db: sessionmaker[Session]) -> None:
    ...
    try:
        ...
        password_record = db_session.query(PasswordTracking).filter(PasswordTracking.id == password_id).first()
        ...
        usage_record = (
            ...
            .filter(PasswordSessionUsage.password_id == password_id, PasswordSessionUsage.session_id == session_id)
            .first()
        )
...
def test_track_password_updates_existing_password_record(test_db: sessionmaker[Session]) -> None:
...
def test_track_password_handles_invalid_timestamp_gracefully(test_db: sessionmaker[Session]) -> None:
...
def test_track_password_handles_nul_bytes_in_password_text(test_db: sessionmaker[Session]) -> None:
...
def test_enrich_session_processes_password_attempts_correctly(test_db: sessionmaker[Session]) -> None:
...
def test_enrich_session_handles_no_password_attempts_gracefully(test_db: sessionmaker[Session]) -> None:
    ...
    events = [
        RawEvent(
            source="test_log.json",
            payload={"eventid": "cowrie.command.input", "timestamp": "2025-01-22T10:00:00Z", "input": "ls -la"},
            session_id="test_session_empty",
...
def test_aggregate_daily_stats_aggregates_password_statistics_correctly(test_db: sessionmaker[Session]) -> None:
    ...
    try:
        ...
        daily_stats = db_session.query(PasswordStatistics).filter(PasswordStatistics.date == target_date).first()
...
def test_aggregate_daily_stats_handles_no_sessions_gracefully(test_db: sessionmaker[Session]) -> None:
...
def test_update_session_persists_enrichment_data_correctly(test_db: sessionmaker[Session]) -> None:
    ...
    session_summary = SessionSummary(
        ...
        command_count=0,
        event_count=1,
        enrichment={"existing_data": {"key": "value"}, "password_stats": {"total_attempts": 5}},
    )
    db_session.add(session_summary)
    db_session.commit()
    ...
    # Flags
    flags = {"vt_flagged": True, "dshield_flagged": False}

    try:
        # When: Update session
        update_session(
            engine=engine, session_id="test_session_update", enrichment_payload=enrichment_payload, flags=flags
        )

        # Then: Verify enrichment data was merged and persisted
        updated_session = (
            db_session.query(SessionSummary).filter(SessionSummary.session_id == "test_session_update").first()
        )
...
def test_update_session_handles_no_existing_enrichment(test_db: sessionmaker[Session]) -> None:
    ...
    # New enrichment payload
    enrichment_payload = {"new_data": {"key": "value"}, "password_stats": {"total_attempts": 5}}

    # Flags
    flags = {"vt_flagged": False, "dshield_flagged": True}

    try:
        # When: Update session
        update_session(
            engine=engine, session_id="test_session_no_enrichment", enrichment_payload=enrichment_payload, flags=flags
        )

        # Then: Verify enrichment data was persisted
        updated_session = (
            db_session.query(SessionSummary).filter(SessionSummary.session_id == "test_session_no_enrichment").first()
        )
...
def test_update_file_persists_virustotal_data_correctly(test_db: sessionmaker[Session]) -> None:
        ...
        "virustotal": {
            "data": {
                "attributes": {
                    ...
                    "last_analysis_stats": {"malicious": 5, "undetected": 10, "harmless": 0},
    ...
    try:
        # When: Update file
        update_file(engine=engine, file_hash="test_file_hash_123", enrichment_payload=enrichment_payload)

        # Then: Verify VirusTotal data was persisted
        updated_file = db_session.query(Files).filter(Files.shasum == "test_file_hash_123").first()
...
def test_update_file_handles_no_virustotal_data_gracefully(test_db: sessionmaker[Session]) -> None:
    ...
    enrichment_payload = {"other_data": {"key": "value"}}

    try:
        # When: Update file
        update_file(engine=engine, file_hash="test_file_hash_456", enrichment_payload=enrichment_payload)

        # Then: Verify file was marked as failed
        updated_file = db_session.query(Files).filter(Files.shasum == "test_file_hash_456").first()
