...
def test_end_to_end_password_enrichment(test_db, cache_manager, mock_rate_limiter):
    ...
    with test_db() as db_session:
        ...
        events = [
            create_test_event(
                1, session_id, 'cowrie.session.connect',
                {'src_ip': '1.2.3.4', 'protocol': 'ssh'}
            ),
            create_test_event(
                2, session_id, 'cowrie.login.failed',
                {'username': 'root', 'password': 'password123'}
            ),
            create_test_event(
                3, session_id, 'cowrie.login.success',
        ...
        # Extract passwords
        loaded_events = db_session.query(RawEvent).filter(
        ...
        password_stats = {
            ...
            'breach_prevalence_max': max(
                (r['prevalence'] for r in checked_passwords.values() if r['breached']),
                default=0
            ),
...
def test_daily_aggregation(test_db, cache_manager, mock_rate_limiter):
    ...
    with test_db() as db_session:
        ...
        total_attempts = sum(
            s.enrichment.get('password_stats', {}).get('total_attempts', 0)
            for s in sessions
        )
        unique_hashes = set()
        for s in sessions:
        ...
        # Verify aggregation
        saved_stats = db_session.query(PasswordStatistics).filter(
...
def test_force_reenrichment(test_db, cache_manager, mock_rate_limiter):
    ...
    with test_db() as db_session:
        ...
        # Verify update
        updated = db_session.query(SessionSummary).filter(
            SessionSummary.session_id == session_id
        ).first()
...
def test_novel_password_tracking(test_db, cache_manager, mock_rate_limiter):
    ...
    with test_db() as db_session:
        ...
        events = [
            create_test_event(
                1, session_id, 'cowrie.login.failed',
                {'username': 'root', 'password': 'breached_pass'}
            ),
            create_test_event(
                2, session_id, 'cowrie.login.failed',
                {'username': 'admin', 'password': 'novel_pass_12345'}
            ),
        ]
        for event in events:
            db_session.add(event)
