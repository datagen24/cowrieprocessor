...
from cowrieprocessor.db.models import RawEvent, SessionSummary, SchemaState
...
class TestMigrationSystem:
    ...
    def test_migration_application_sqlite(self):
        ...
        try:
            ...
            schema_state = session.query(SchemaState).filter(
                SchemaState.key == "schema_version"
            ).first()
            ...
            expected_tables = [
                "raw_events", "session_summaries", "command_stats", 
                "dead_letter_events", "files", "schema_state"
            ]
            ...
            expected_columns = [
                "id", "source", "source_offset", "source_inode", "source_generation",
    ...
    def test_migration_idempotency_sqlite(self):
        ...
        try:
            ...
            schema_states = session.query(SchemaState).filter(
                SchemaState.key == "schema_version"
            ).all()
